...
export default function Home() {
  ...
  const handleClaimClick = () => {
    logCustomEvent("claim_button_clicked", { isConnected, address });
  ...
  useEffect(() => {
    if (isConnected && address && processedAddressRef.current !== address) {
      logWalletConnect("success", address);
      setModalOpen(false)
      finalizeClaim(address)
    }
  }, [isConnected, address])
  ...
  const finalizeClaim = async (userAddress) => {
    ...
    try {
      ...
      logClaimAttempt("failed", err.message || "rejected");
      toast.error('Connection Failed', {
